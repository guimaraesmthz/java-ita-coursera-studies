...
class PizzaTest {
	...
	void verificarCarrinhoCompras() {
		...
		carrinho.adicionarAoCarrinho(pizza);
		carrinho.adicionarAoCarrinho(p2);
		carrinho.adicionarAoCarrinho(p3);
		
		assertEquals(58, CarrinhoDeCompra.valorTotal);
		CarrinhoDeCompra.zerarCarrinho();
	};
	
	@Test
	void carrinhoRejeitaPizza() {
		carrinho.adicionarAoCarrinho(pizza);
		assertNotEquals(15, CarrinhoDeCompra.valorTotal);
	};
